...
<div class="container">
  ...
  su1변수값을 출력1 <c:out value="${su1}"/><br/> <!--변수를 출력하고 싶을때는 꼭 이엘 써줌  -->
  su1변수값을 출력2 ${su1}<br/>
  수식을 출력1? <c:out value="${100 * 200}"/><br/> 
  수식을 출력2? <c:out value="${'100' + 200}"/><br/> 
  수식을 출력3? <c:out value="${'100' + '200'}"/><br/> <!--숫자로 인식  -->
  <% String name = "홍길동"; %>
  값을 출력1? <%=name %><br/>
  값을 출력2? <c:out value='<%=name %>'/><br/>
  값을 출력3? <c:out value='${name}'/> : 저장소에 저장하지 않은 변수는 값이 출력되지 않는다. <br/>
  <% pageContext.setAttribute("name", name); %>
  값을 출력5? <c:out value='${name}'/> == ${name}<br/>
  su1변수값을 출력1 ${su1}<br/><br/>
  su1변수를 제거? <c:remove var="su1"/><br/>
  su1변수값을 출력2 ${su1}<br/><br/>
  su1변수에 name변수값을 저장 <c:set var="su1" value="${name}"></c:set>
  ...
  <p>단점1 : jstl의 숫자비교는 문자형식으로 비교한다.</p>
  <p>단점2 : EL조건문에는 Else가 없다.</p>
  <p>사용예 : (su1=300, su2=400)</p>
  ...
  <div>14. su3 < su4 : <c:if test="${su3 < su4}">14.su3가 su4보다 작다.</c:if></div> <!--문자로 비교라 1보다4가 크다고 보기 때문에 이게 참으로 나온다  -->
  <div>113. su3 > su4 : <c:if test="${su3+0 > su4+0}">113.su3가 su4보다 크다.</c:if></div><!--숫자 비교하고 싶을때는 뒤에 +0을 해준다  -->
  ...
  <div>예제 : URL에 jumsu를 QUeryString방식으로 입력후 학점을 구하시오</div>
  ...
  <div>예제 : URL에 직급코드(code)를 QUeryString방식으로 입력후 직급명을 구하시오</div>
