...
<head>
  ...
  <script>
    ...
    // 회원 개별 조회
    function heowonSearch(idx) {
    	$.ajax({
    		url  : "${ctp}/hoewonSearch.st",
    		type : "get",
    		data : {idx : idx},
    		success:function(res) {
    			let str = res.split("/");
...
<div class="container">
  ...
  <form name="myform">
    <table class="table table-bordered">
      ...
      <tr>
        <th>성명</th>
        <td><input type="text" name="name" id="name" class="form-contorl"/></td>
      </tr>
      <tr>
        <th>나이</th>
        <td><input type="number" name="age" id="age" class="form-contorl"/></td>
      </tr>
      <tr>
        <th>성별</th>
        <td>
      ...
      <tr>
        <td colspan="2" class="text-center">
          <input type="button" value="회원등록" oncilck="hInput()" class="btn btn-success mr-2"/>
          <input type="reset" value="다시입력" class="btn btn-primary mr-2"/>
          <input type="button" value="회원수정" oncilck="hUpdate()" class="btn btn-info mr-2"/>
          <input type="button" value="전체보기" oncilck="location.href='hoewonList.st';" class="btn btn-secondary"/>
        </td>
      </tr>
    </table>
  </form>
	  ...
	  <table class="table table-hover text-center">
	    ...
	    <c:forEach var="vo" items="${vos}" varStatus="st">
	      <tr>
	        ...
	        <td>${vo.address}</td>
	        <td>
	          <a href="javascript:heowonSearch(${vo.idx})" class="btn btn-success btn-sm">개별조회</a>
	          <a href="javascript:heowonDelete(${vo.idx})" class="btn btn-danger btn-sm">삭제</a>
	        </td>
	      </tr>
	    </c:forEach>
